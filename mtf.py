...
Commands:
  -a, --add, create        Create a new MTF file from the specified input.
  -x, --extract, extract   Extract files from an MTF to a specified directory.
  -l, --list, list         List the contents to a file (use with -l option)

Options:
  -i, --input    Specify the input MTF file.
  -o, --output   Specify the output directory.
  -l             Specify the output file for list of content.
...
def main():
    ...
    if command == "create":
        ...
        print(f"Error: Invalid subcommand '{command}'.")
        if is_windows():
            input("Press Enter to exit...")
        else:
            os.system('read -p "Press Enter to exit..." -n1 -s; echo ""')
